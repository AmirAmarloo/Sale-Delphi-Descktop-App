object frmEGoodsDiscount: TfrmEGoodsDiscount
  Left = 323
  Top = 9
  Width = 527
  Height = 553
  BiDiMode = bdRightToLeft
  Caption = #1578#1582#1601#1610#1601#1575#1578' '#1585#1608#1610' '#1705#1575#1604#1575' - '#1585#1610#1575#1604#1610
  Color = clBtnFace
  Font.Charset = ARABIC_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsStayOnTop
  KeyPreview = True
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object panButton: TPanel
    Left = 0
    Top = 477
    Width = 519
    Height = 49
    Align = alBottom
    TabOrder = 2
    DesignSize = (
      519
      49)
    object sbLast: TSpeedButton
      Left = 86
      Top = 3
      Width = 25
      Height = 25
      Hint = #1575#1606#1578#1607#1575
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbLastClick
    end
    object sbNext: TSpeedButton
      Left = 60
      Top = 3
      Width = 25
      Height = 25
      Hint = #1576#1593#1583#1610
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Margin = 5
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbNextClick
    end
    object sbPrev: TSpeedButton
      Left = 34
      Top = 3
      Width = 25
      Height = 25
      Hint = #1602#1576#1604#1610
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Margin = 1
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbPrevClick
    end
    object sbFirst: TSpeedButton
      Left = 8
      Top = 3
      Width = 25
      Height = 25
      Hint = #1575#1576#1578#1583#1575
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      Margin = 2
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbFirstClick
    end
    object sbExit: TSpeedButton
      Left = 300
      Top = 3
      Width = 65
      Height = 25
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      Caption = #1582#1585#1608#1580
      Glyph.Data = {
        
        0333337F777FFFFF7F3333000000000003333377777777777333}
      NumGlyphs = 2
      ParentBiDiMode = False
      OnClick = sbExitClick
    end
    object sbExportToExcel: TSpeedButton
      Left = 270
      Top = 4
      Width = 24
      Height = 25
      Hint = #1575#1585#1587#1575#1604' '#1576#1607' '#1575#1705#1587#1604
      Anchors = [akTop, akRight]
      Glyph.Data = {
        
        3333777777333333333333333333333333333333333333333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = sbExportToExcelClick
    end
    object stbMain: TStatusBar
      Left = 1
      Top = 29
      Width = 517
      Height = 19
      Constraints.MaxHeight = 19
      Constraints.MinHeight = 19
      Panels = <>
      SimplePanel = True
      SimpleText = 
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9 +
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9 +
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9 +
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9 +
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9 +
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9 +
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9 +
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9#9 +
        #9#9#9#9#9#9#9#9#9#9#9#9#9#9#1576#1607' '#1578#1585#1578#1610#1576' :'
    end
    object BtnPost: TBitBtn
      Tag = 24070016
      Left = 439
      Top = 3
      Width = 65
      Height = 25
      Anchors = [akTop, akRight]
      Caption = #1579#1576#1578
      TabOrder = 0
      OnClick = BtnPostClick
      Glyph.Data = {
        
        5990555555555555577755555555555555555555555555555555}
      NumGlyphs = 2
    end
    object BtnDel: TBitBtn
      Tag = 24070017
      Left = 369
      Top = 3
      Width = 65
      Height = 25
      Anchors = [akTop, akRight]
      Caption = #1581#1584#1601
      TabOrder = 1
      OnClick = BtnDelClick
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 519
    Height = 5
    Align = alTop
    TabOrder = 0
    Visible = False
    DesignSize = (
      519
      5)
    object Label1: TLabel
      Left = 474
      Top = 16
      Width = 29
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1705#1583' '#1705#1575#1604#1575
    end
    object sbGood: TSpeedButton
      Left = 23
      Top = 12
      Width = 21
      Height = 20
      Hint = #1601#1607#1585#1587#1578
      Anchors = [akTop, akRight]
      Glyph.Data = {
        
        FFF000000000000000000F0CCCCCCCC0F0F00000000000000000}
      OnClick = sbGoodClick
    end
    object edGdsCode1: TEdit
      Left = 358
      Top = 12
      Width = 113
      Height = 21
      Anchors = [akTop, akRight]
      TabOrder = 0
      OnExit = edGdsCode1Exit
    end
    object edGdName: TEdit
      Left = 41
      Top = 12
      Width = 312
      Height = 21
      Color = clInfoBk
      Enabled = False
      TabOrder = 1
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 5
    Width = 519
    Height = 472
    Align = alClient
    DataSource = dsEGoodsDiscount
    TabOrder = 1
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnEditButtonClick = DBGrid1EditButtonClick
    Columns = <
      item
        Expanded = False
        FieldName = 'GD_MIN_QTY'
        Width = 118
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'GD_MAX_QTY'
        Width = 108
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'GD_DATE'
        Width = 100
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'GD_TODATE'
        Width = 90
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'GD_RATE'
        Width = 76
        Visible = True
      end>
  end
  object qrEGoodsDiscount: TQuery
    CachedUpdates = True
    BeforePost = qrEGoodsDiscountBeforePost
    AfterScroll = qrEGoodsDiscountAfterScroll
    OnCalcFields = qrEGoodsDiscountCalcFields
    OnNewRecord = qrEGoodsDiscountNewRecord
    DatabaseName = 'SALES'
    SQL.Strings = (
      'SELECT * FROM GOODSDICOUNT_T')
    UpdateObject = usEGoodsDiscount
    Left = 104
    Top = 104
    object qrEGoodsDiscountGD_DATE: TStringField
      DisplayLabel = #1575#1586' '#1578#1575#1585#1610#1582
      FieldName = 'GD_DATE'
      Origin = 'SALES.GOODSDICOUNT_T.GD_DATE'
      OnValidate = qrEGoodsDiscountGD_DATEValidate
      EditMask = '!9999/99/99;0;_'
      FixedChar = True
      Size = 8
    end
    object qrEGoodsDiscountGD_RATE: TFloatField
      DisplayLabel = #1583#1585#1589#1583' '#1578#1582#1601#1610#1601
      FieldName = 'GD_RATE'
      Origin = 'SALES.GOODSDICOUNT_T.GD_RATE'
      OnValidate = qrEGoodsDiscountGD_RATEValidate
    end
    object qrEGoodsDiscountCF_GOODNAME: TStringField
      DisplayLabel = #1588#1585#1581' '#1705#1575#1604#1575
      FieldKind = fkCalculated
      FieldName = 'CF_GOODNAME'
      Size = 100
      Calculated = True
    end
    object qrEGoodsDiscountGD_MIN_QTY: TFloatField
      DisplayLabel = #1575#1586' '#1605#1576#1604#1594
      FieldName = 'GD_MIN_QTY'
      Origin = 'SALES.GOODSDICOUNT_T.GD_MIN_QTY'
      OnValidate = qrEGoodsDiscountGD_MIN_QTYValidate
      DisplayFormat = '#,'
      EditFormat = '#'
    end
    object qrEGoodsDiscountGD_MAX_QTY: TFloatField
      DisplayLabel = #1578#1575' '#1605#1576#1604#1594
      FieldName = 'GD_MAX_QTY'
      Origin = 'SALES.GOODSDICOUNT_T.GD_MAX_QTY'
      OnValidate = qrEGoodsDiscountGD_MAX_QTYValidate
      DisplayFormat = '#,'
      EditFormat = '#'
    end
    object qrEGoodsDiscountGD_TODATE: TStringField
      DisplayLabel = #1578#1575' '#1578#1575#1585#1610#1582
      FieldName = 'GD_TODATE'
      Origin = 'SALES.GOODSDICOUNT_T.GD_TODATE'
      EditMask = '!9999/99/99;0;_'
      FixedChar = True
      Size = 8
    end
    object qrEGoodsDiscountGD_GDSINFO: TFloatField
      DisplayLabel = #1705#1583' '#1605#1588#1582#1589#1607' '#1705#1575#1604#1575
      FieldName = 'GD_GDSINFO'
      Origin = 'SALES.GOODSDICOUNT_T.GD_GDSINFO'
      OnValidate = qrEGoodsDiscountGD_GDSINFOValidate
    end
    object qrEGoodsDiscountCF_FDSINFODESC: TStringField
      DisplayLabel = #1588#1585#1581' '#1605#1588#1582#1589#1607' '#1705#1575#1604#1575
      FieldKind = fkCalculated
      FieldName = 'CF_FDSINFODESC'
      Size = 50
      Calculated = True
    end
  end
  object dsEGoodsDiscount: TDataSource
    DataSet = qrEGoodsDiscount
    Left = 168
    Top = 104
  end
  object usEGoodsDiscount: TUpdateSQL
    ModifySQL.Strings = (
      'update GOODSDICOUNT_T'
      'set'
      '  GD_MIN_QTY = :GD_MIN_QTY,'
      '  GD_MAX_QTY = :GD_MAX_QTY,'
      '  GD_DATE = :GD_DATE,'
      '  GD_RATE = :GD_RATE,'
      '  GD_TODATE = :GD_TODATE,'
      '  GD_GDSINFO = :GD_GDSINFO'
      'where'
      '  GD_MIN_QTY = :OLD_GD_MIN_QTY and'
      '  GD_DATE = :OLD_GD_DATE')
    InsertSQL.Strings = (
      'insert into GOODSDICOUNT_T'
      
        '  (GD_MIN_QTY, GD_MAX_QTY, GD_DATE, GD_RATE, GD_TODATE, GD_GDSIN' +
        'FO)'
      'values'
      
        '  (:GD_MIN_QTY, :GD_MAX_QTY, :GD_DATE, :GD_RATE, :GD_TODATE, :GD' +
        '_GDSINFO)')
    DeleteSQL.Strings = (
      'delete from GOODSDICOUNT_T'
      'where'
      '  GD_MIN_QTY = :OLD_GD_MIN_QTY and'
      '  GD_DATE = :OLD_GD_DATE')
    Left = 136
    Top = 104
  end
end
