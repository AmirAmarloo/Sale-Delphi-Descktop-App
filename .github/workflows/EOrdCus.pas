...
begin
  ...
  begin
    ...
    if CheckFactor(FDM_ExtSales.qrEOrderDet.FieldByName('GDS_CODE').AsString) then
    begin
      MbError(SendFactor);
      Abort;
    end;
...
begin
  ...
  begin
    ...
    if Sender.AsFloat > FDM_ExtSales.qrEOrderDet.FieldByName('ORDQTY').AsFloat then Exit;
(*
    if (Sender.AsFloat < CheckQTY(FDM_ExtSales.qrEOrderDet.FieldByName('GDS_CODE').AsString)) then
    begin
      MbError(LowThanOrdDelv);
      Abort;
    end;
*)    
  end;
...
{
  if not MsDateIsValid1(FDM_ExtSales.qrEOrder.FieldByName('ORDDATE').AsString, False) then
  begin
    MbMessage('ÊÇÑíÎ ÛíÑ ãÌÇÒ', mtError, [mbOk], mbOk);
    dbedOrdDte.SetFocus;
...
//        Delv_Ord := FDM_ExtSales.qrEOrder.FieldByName('Ord_Delv_No').AsInteger;
...
procedure TFrmEOrdCus.UpdateSTAT;
begin
  with FDM_ExtSales.qrETemp, SQL do
  begin
    Clear;
...
procedure TFrmEOrdCus.DetBeforeDelete(DataSet: TDataSet);
begin
(*
  with DataSet do
  begin
...
function TFrmEOrdCus.CheckGSD_CODE(GCode: String): Boolean;
begin
(*
  with FDM_ExtSales.qrETemp, SQL do
  begin
    ...
    Add('   AND M.DOC_M_SALEORNO = :ORDNO            ');
...
function TFrmEOrdCus.CheckQTY(GCode: String): Integer;
begin
(*
  with FDM_ExtSales.qrETemp, SQL do
  begin
...
function TFrmEOrdCus.CheckFactor(GCode: String): Boolean;
begin
  with FDM_ExtSales.qrETemp, SQL do
  begin
    Clear;
    Add('SELECT T.INVSRL                         ');
    Add('  FROM INVOICEPRDT_T T, INVOICEPRDR_T R ');
    Add(' WHERE T.INVSRL = R.INVSRL              ');
    Add('   AND ORDNO    = :ORDNO                ');
    Add('   AND GDS_CODE = :GDS_CODE             ');
    ParamByName('ORDNO').AsString    := dbedOrdNo.Text;
    ParamByName('GDS_CODE').AsString := GCode;
